...
export const MAX_FILE_SIZE = 5 * 1024 * 1024 // 5MB — kept for backward compat
export const MAX_RAW_FILE_SIZE = 5 * 1024 * 1024 // 5MB — matches Supabase bucket limit
...
/**
 * Demo stub — used only when the editor is not in journal mode.
 * Simulates upload progress and returns a placeholder image URL.
 */
...
/**
 * Creates a real image upload handler scoped to a specific journal and user.
 * Validates MIME type and size, compresses the image, then uploads to Supabase Storage.
 *
 * @param {string} journalId - The journal the image belongs to
 * @param {string} userId - The authenticated user's ID
 * @returns An async upload function compatible with ImageUploadNode
 */
export function createImageUploadHandler(journalId, userId) {
  return async (file, onProgress, abortSignal) => {
    ...
    if (file.size > MAX_RAW_FILE_SIZE) {
      throw new Error(`File size exceeds ${MAX_RAW_FILE_SIZE / (1024 * 1024)}MB limit`)
    }

    // Capture MIME type before compression — browser-image-compression may return
    // a Blob with an empty or altered type property
    ...
    // Lazy import to avoid pulling Supabase into the shared tiptap-utils bundle
    const { uploadJournalImage } = await import("@/features/journals/apis")

    const url = await uploadJournalImage(compressedFile, userId, journalId, originalMimeType)
