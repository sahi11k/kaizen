...
function buildDayMap(items, getDate) {
  const map = {};
  items.forEach((item) => {
...
const HeatmapRow = ({ label, levels }) => (
  <div className="grid gap-2 items-center" style={{ gridTemplateColumns: "68px repeat(7, 1fr)" }}>
    <span className="text-xs text-muted-foreground pr-1.5 leading-snug">{label}</span>
    {levels.map((lvl, i) => (
...
const EngagementHeatmap = () => {
  ...
  const sessionsByDay = useMemo(() => buildDayMap(sessions, (s) => s.createdAt), [sessions]);
  const journalsByDay = useMemo(() => buildDayMap(journals, (j) => j.createdAt || j.date), [journals]);
  const tasksByDay = useMemo(() => buildDayMap(completedTasks, (t) => t.updatedAt), [completedTasks]);
  ...
  return (
    <Card title="7-Day Engagement" className="h-full !pb-4" contentClassName="flex h-full flex-col">
      {hasActivity ? (
        <>
          <div className="grid gap-2 mb-1.5 items-center" style={{ gridTemplateColumns: "68px repeat(7, 1fr)" }}>
            <div />
            {dayLabels.map((d, i) => (
              <div key={i} className="text-center text-xs text-muted-foreground tracking-wide">
                {d}
              </div>
            ))}
          </div>
